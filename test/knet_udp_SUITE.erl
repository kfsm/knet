...
gen_udp_client_to_knet_server(_) ->
   {ok, LSock} = knet_echo_sock:init("udp://*:8888", #{}),
   {ok,  Sock} = gen_udp:open(0, [binary, {active, false}]),
   ok = gen_udp:send(Sock, "127.0.0.1", 8888, <<"-123456">>),
   {ok, {{127,0,0,1}, 8888, <<"+123456">>}} = gen_udp:recv(Sock, 0, 5000),
   gen_udp:close(Sock),
   knet:close(LSock).
...
knet_client_to_knet_server(_) ->
   {ok, LSock} = knet_echo_sock:init("udp://*:8888", #{}),
   {ok, Sock}  = knet:connect("udp://127.0.0.1:8888"),
   {ioctl, b, Sock} = knet:recv(Sock),
   {udp, Sock, {established, _}} = knet:recv(Sock),
   ok = knet:send(Sock, <<"-123456">>),
   {udp, Sock, {{127,0,0,1}, 8888, <<"+123456">>}} = knet:recv(Sock),
   knet:close(Sock),
   knet:close(LSock).
